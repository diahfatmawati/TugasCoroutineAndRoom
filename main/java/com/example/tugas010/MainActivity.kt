...
import com.example.tugas010.note.Note
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actionBar!!.title = "Catatan Pesanan Toko Mudjiati"
        ...
        ItemTouchHelper(object :
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                Toast.makeText(baseContext, "Pesanan sudah diantarkan. Pesanan dihapus!", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item?.itemId) {
            R.id.delete_all_notes -> {
                ...
                Toast.makeText(this, "Semua pesanan sudah dihapus!", Toast.LENGTH_SHORT).show()
                true
            }
            else -> {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ADD_NOTE_REQUEST && resultCode == Activity.RESULT_OK) {
            ...
            if (id == -1) {
                Toast.makeText(this, "Pembaharuan pesanan gagal!", Toast.LENGTH_SHORT).show()
            }
            val updateNote = Note(
                data!!.getStringExtra(AddEditNoteActivity.EXTRA_JUDUL).toString(),
                data.getStringExtra(AddEditNoteActivity.EXTRA_DESKRIPSI).toString(),
